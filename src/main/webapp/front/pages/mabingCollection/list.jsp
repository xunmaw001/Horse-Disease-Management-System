            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">马病收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">马病编号</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="mabingUuidNumber" id="mabingUuidNumber" placeholder="马病编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">马病名称</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="mabingName" id="mabingName" placeholder="马病名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                         
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">病因类型</label>
                              <div class="layui-input-inline">
                                  <select name="mabingTypes" id="mabingTypes" lay-filter="mabingTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in mabingTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">基本症状</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="zhengzhuangText" id="zhengzhuangText" placeholder="基本症状" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="zhiliaofanganText" id="zhiliaofanganText" placeholder="治疗方案" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">预防方案</label>
                              <div class="layui-input-inline">
                                  <input type="text" name="yufangfanganText" id="yufangfanganText" placeholder="预防方案" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <div class="team-info">
                            <div class="team-row">
                                <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-6 col-xs-6 team-grids">
                                    <i @click="deleteMabingCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                    <div class="team-agile-img" @click="jumpCheck('../mabing/detail.jsp?id='+item.mabingId , item.mabingDelete == null?'':item.mabingDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img :src="item.mabingPhoto?item.mabingPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                        <div class="view-caption">
                                            <div class="w3ls-info">
                                                <h4>{{item.mabingName}}</h4>
                                            </div>
                                            <b v-if="item.mabingNewMoney" style="float: right;display: inline-block;color: #FFFFFF;font-size: 17px;">
                                                {{item.mabingNewMoney}} RMB
                                            </b>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            mabingCollectionTypesList : [],
        // 级联表字典表数据
            mabingTypesList : [],
            shangxiaTypesList : [],
            mabingCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteMabingCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('mabingCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function mabingCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=mabing_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.mabingCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function mabingTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=mabing_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.mabingTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#mabingUuidNumber').val() != null && jquery('#mabingUuidNumber').val() != "" && jquery('#mabingUuidNumber').val() != "null" ) {
                param['mabingUuidNumber'] = jquery('#mabingUuidNumber').val();
            }
                     
            if (jquery('#mabingName').val() != null && jquery('#mabingName').val() != "" && jquery('#mabingName').val() != "null" ) {
                param['mabingName'] = jquery('#mabingName').val();
            }
                                         
            var mabingTypes = $("#mabingTypes").find("option:selected").val();
            if (mabingTypes != null && mabingTypes != "" && mabingTypes != "null") {
                            param['mabingTypes'] = mabingTypes;
                        }
                     
            if (jquery('#zhengzhuangText').val() != null && jquery('#zhengzhuangText').val() != "" && jquery('#zhengzhuangText').val() != "null" ) {
                param['zhengzhuangText'] = jquery('#zhengzhuangText').val();
            }
                     
            if (jquery('#zhiliaofanganText').val() != null && jquery('#zhiliaofanganText').val() != "" && jquery('#zhiliaofanganText').val() != "null" ) {
                param['zhiliaofanganText'] = jquery('#zhiliaofanganText').val();
            }
                     
            if (jquery('#yufangfanganText').val() != null && jquery('#yufangfanganText').val() != "" && jquery('#yufangfanganText').val() != "null" ) {
                param['yufangfanganText'] = jquery('#yufangfanganText').val();
            }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['mabingCollectionTypes'] = 1;

            // 获取列表数据
            http.request('mabingCollection/page', 'get', param, function(res) {
