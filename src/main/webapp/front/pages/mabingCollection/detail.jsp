...
<body>
        ...
        <section class="blog-details-section pt-120 pb-120">
            <div class="data-detail-breadcrumb" style="width: 1110px; padding: 0px 10px; box-shadow: rgba(255, 0, 0, 0.3) 0px 0px 0px; margin: 20px auto; border-color: rgb(135, 206, 250); border-radius: 4px; border-width: 0px; border-style: solid; height: 54px;">
                <span class="layui-breadcrumb">
                    ...
                    <a><cite>{{detail.mabingCollectionName}}</cite></a>
                </span>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="blog-details-wrapper">
                    <div><img :src="detail.mabingCollectionPhoto" alt="image" style="width: 80%;height: auto" ></div>
                    <div class="blog-details-header">
                        <h3 class="blog-details-title">{{title}}</h3>
                        <ul class="post-meta">
                            <li v-if="detail.mabingCollectionTypes">类型：
                                {{detail.mabingCollectionValue}}
                            </li>
                        </ul>
        ...
        <div class="col-lg-4" v-if="mabingCollectionRecommendList">
            <div class="sidebar">
                ...
                <div class="widget">
                    ...
                    <ul class="small-post-list">
                        <li class="small-post-single" v-for="(item,index) in mabingCollectionRecommendList" v-bind:key="index" @click="jump('../mabingCollection/detail.jsp?id='+item.id)">
                            <div class="thumb"><img width="65px" :src="item.mabingCollectionPhoto"  alt="image"></div>
                            <div class="content">
                                <a class="main_color" class="date">{{item.mabingCollectionName}}</a>
                                <h6 v-if="item.mabingCollectionNewMoney" class="post-title"><a style="color: red">{{item.mabingCollectionNewMoney}}</a>RMB</h6>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            //系统推荐
            mabingCollectionRecommendList: [],
            dataList: [],
            // 当前详情页表
            detailTable: 'mabingCollection',
        },
        //  清除定时器
        destroyed: function () {
            // 不知道具体作用
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage'], function () {
        ...
        http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
            // 详情信息
            vue.detail = res.data;
            vue.title = vue.detail.mabingCollectionName;
           // 轮播图片
            vue.swiperList = vue.detail.mabingCollectionPhoto ? vue.detail.mabingCollectionPhoto.split(",") : [];
            var swiperItemHtml = '';
        ...
        http.request(`mabingCollection/list`, 'get', {
            page: 1,
            limit: 5,
            mabingCollectionTypes: vue.detail.mabingCollectionTypes,




        }, function (res) {
            vue.mabingCollectionRecommendList = res.data.list;
        });
